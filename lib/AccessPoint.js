...
class AccessPoint {
  constructor(port, handler, callback) {
    ...
    http.createServer((req, res) => {
      ...
      } else if (basePath === 'invoke') {
        if ((method === 'GET') && (parts.length)) {
          this.handler(res, this.TYPE_INVOKE_NO_PARAM(parts[0], req.query, TYPE_ROUTED));
        } else if ((method === 'POST') && (parts.length)) {
          this.handler(res, this.TYPE_INVOKE_WITH_PARAMS(parts[0], req.query, TYPE_ROUTED));
        }
      } else if (basePath === 'routes') {
        if ((method === 'DELETE') && (parts.length)) {
          this.handler(res, this.TYPE_ROUTE_UNDEPLOY(parts[0]));
        } else if ((method === 'PUT') && (parts.length)) {
          this.handler(res, this.TYPE_ROUTE_DEPLOY(parts[0], parts[1]));
        }
      } else {
        res.statusCode = 404;
  ...
  TYPE_ROUTE_DEPLOY(route, digest) {
    return {
      type: 'ROUTE_DEPLOY',
      route,
      digest
